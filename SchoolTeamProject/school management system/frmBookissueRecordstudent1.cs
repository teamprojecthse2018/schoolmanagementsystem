...
{
    public partial class frmBookissueRecordstudent1 : Form
    {
        ...
        public frmBookissueRecordstudent1()
        {
            InitializeComponent();
        }

        private void frmBookissueRecordstudent1_Load(object sender, EventArgs e)
        {
            AutocompleteStudent();
        ...
        private void frmBookissueRecordstudent1_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Hide();
        ...
        private void button10_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                con.Open();
                cmd = new SqlCommand("SELECT Rtrim(TransactionID) as [Transaction ID],(IssueDate) as [Issue Date],(DueDate) as [Due Date],Rtrim(Book.AccessionNo) as [Accession No],Rtrim(BookTitle) as [Book Title],Rtrim(Author) [Author],Rtrim(Subject) [Suject],Rtrim(ISBN) [ISBN],Rtrim(Edition) [Edition],Rtrim(Student.AdmissionNo) as [AdmissionNo],Rtrim(StudentName) as [Student Name],Rtrim(Student.Class) [Class],Rtrim(Student.Section) [Section], Rtrim(studentBookissue.Status) [Status] from Book,StudentBookIssue,Student where Book.AccessionNo=StudentBookIssue.AccessionNo and studentBookIssue.AdmissionNo=Student.AdmissionNo and IssueDate Between @date1 and @date2 order by IssueDate", con);
